 ...
 * 
 * Características:
 * - Carregamento dinâmico das configurações do /api/config
...
(function(window) {
  ...
  function log(message, data = null, level = 'info') {
    ...
    const prefix = `[FB-PIXEL-MANAGER-PRIVACY]`;
  ...
  async function loadFacebookConfig(retryCount = 0) {
    try {
      ...
      if (!config.facebook || !config.facebook.pixelId) {
        throw new Error('FB_PIXEL_ID não encontrado na configuração do servidor');
      }

      // Atualizar configuração global
      FBPixelManager.config.FB_PIXEL_ID = config.facebook.pixelId;
      FBPixelManager.config.FB_PIXEL_TOKEN = config.facebook.pixelToken || '';
      FBPixelManager.config.FB_TEST_EVENT_CODE = config.facebook.testEventCode || 'TEST74140';
      FBPixelManager.config.FORCE_FB_TEST_MODE = config.facebook.forceTestMode || false;
      ...
      log('Configurações do Facebook Pixel carregadas com sucesso', {
        pixelId: config.facebook.pixelId ? 'DEFINIDO' : 'NÃO DEFINIDO',
        testMode: config.facebook.forceTestMode,
        testEventCode: config.facebook.testEventCode ? 'DEFINIDO' : 'NÃO DEFINIDO'
      }, 'success');
  ...
  async function initialize() {
    try {
      log('🚀 Iniciando Facebook Pixel Manager (Privacy)...');
      ...
      log('🎉 Facebook Pixel Manager (Privacy) inicializado com sucesso!', {
  ...
  log('Facebook Pixel Manager (Privacy) carregado. Aguardando inicialização...', null, 'debug');
